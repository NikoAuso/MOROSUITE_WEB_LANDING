...
3. **`src/lib/api.ts`** — the only network layer. `fetchJson` does **per-process memoization** + retry-with-backoff (`config.fetch.retries`/`retryDelayMs`/`timeoutMs`). `fetchJsonSafe` wraps it and **catches errors**, returning the provided fallback so a missing backend never breaks the build. All wrappers (`api.site`, `api.openingHours`, `api.pricing`, `api.legal(doc)`, `api.transparency`, `api.seoStructuredData`) fall back to a `PLACEHOLDER_*` value from `src/lib/placeholders.ts` and always resolve to a real payload (no `null`s). Calling `api.site()` in both a page and its layout costs one HTTP round-trip per build thanks to memoization.
4. **`src/lib/placeholders.ts`** — **single source of truth for offline/demo content**. Holds `PLACEHOLDER_SITE`, `PLACEHOLDER_OPENING_HOURS`, `PLACEHOLDER_PRICING`, `PLACEHOLDER_TRANSPARENCY`, `PLACEHOLDER_SEO` and `PLACEHOLDER_LEGAL` (keyed by `LegalDocumentName`). Edit this file to tweak what users see when the backend is down — strings, demo hours, demo pricing rows, demo legal markdown body, etc.
...
7. **`src/layouts/PublicLayout.astro`** — `<head>` (canonical, OG, JSON-LD, GA4 consent-mode v2 bootstrap, Bunny Fonts, Font Awesome CDN) + `<Header>` / `<Footer>` / `<CookieBanner>` / `<CookiePreferencesToggle>`. Every page should go through this.
8. **`src/layouts/LegalDocument.astro`** — wraps `PublicLayout` and renders an `api.legal(doc)` payload (markdown → HTML via `src/lib/markdown.ts`). Used by `terms.astro`, `policy.astro`, `cookie.astro`, `regolamento.astro` — each is a 4-line page that just calls `api.legal('<name>')` (the API layer guarantees a payload thanks to placeholders).
